...
export default function SamePeopleMoreOutput() {
  ...
  return (
    <section ref={ref} className="relative border-t border-b border-stone-200 bg-[#2D3A4A]">
      ...
      <div className="md:hidden absolute left-0 right-0 top-0 h-1" />
        ...
        >
          ...
          <div className="hidden md:grid grid-cols-3 gap-5 ">
            {blocks.map((block, i) => (
              <div
                key={`d-${i}`}
                className={`bg-white px-6 py-8 rounded-[20px] shadow-[0_4px_4px_0_rgba(66,99,235,0.5)] transition-all duration-500 transform hover:scale-105 ${
                  isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'
                }`}
          ...
          <div className="md:hidden">
            {blocks.map((block, i) => (
              <div
                key={`m-${i}`}
                className={`py-8 ${i < blocks.length - 1 ? 'border-b border-stone-200' : ''} transition-all duration-500 ${
                  isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'
                }`}
                style={{ transitionDelay: `${(i + 1) * 120}ms` }}
              >
