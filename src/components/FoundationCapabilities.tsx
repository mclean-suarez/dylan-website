        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
          {capabilities.map((cap, i) => (
            <div
                ...
                className={`bg-stone-50 p-5 md:p-6 relative transition-all duration-500 border rounded-[20px] shadow-[0_4px_4px_0_rgba(66,99,235,0.5)] ${
                  hoveredIndex === i ? 'border-brand-700' : 'border-stone-200'
                } ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'} ${
              ...
              >
                <span
                  className={`font-mono text-3xl font-bold tracking-tight block mb-2 transition-colors duration-300 ${
                    hoveredIndex === i ? 'text-brand-700' : 'text-stone-200'
                  }`}
                >
                  {cap.number}
                </span>
                <h3 className="text-sm font-bold text-stone-900 leading-snug">{cap.title}</h3>
              </div>

              {/* Capability signal */}
              <div className="mt-4 relative h-2">
                {/* Line */}
                <div className="absolute top-1/2 left-0 right-0 h-px bg-stone-200 transform -translate-y-1/2" />
